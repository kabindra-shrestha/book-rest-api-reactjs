...
export default class BookList extends Component {
    ...
    findAllBooks(currentPage) {
        ...
        fetch("http://localhost:8080/rest/books?pageNumber=" + currentPage + "&pageSize=" + this.state.booksPerPage + "&sortBy=" + "title" + "&sortDir=" + sortDir)
    ...
    searchData = (currentPage) => {
        currentPage -= 1;
        fetch("http://localhost:8080/rest/books/search/" + this.state.search + "?page=" + currentPage + "&size=" + this.state.booksPerPage)
            .then(response => response.json())
            .then((data) => {
                this.setState({
                    books: data.content,
                    ...
                    <Card.Body>
                        <Table bordered hover striped variant="dark">
                            ...
                            <tr>
                                ...
                                <th>Author</th>
                                <th>ISBN Number</th>
                                <th onClick={this.sortData}>Price <div
                                    className={this.state.sortToggle ? "arrow arrow-down" : "arrow arrow-up"}></div>
                                </th>
                                <th>Language</th>
                                <th>Genre</th>
                                <th>Actions</th>
